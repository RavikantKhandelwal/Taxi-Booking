...
def booking_history(request, id):
    ...
    booking_history = Booking.objects.filter(user=user)
    serial_booking_history = BookingSerializer(booking_history, many=True)
    return Response(serial_booking_history)
...
def book_taxi(request):
    #try:
        ...
        with transaction.atomic():
            driver = Driver.objects.select_for_update().filter(available=True)[0]
